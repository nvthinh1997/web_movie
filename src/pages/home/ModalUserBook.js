...
export default function ModalUserBook(props) {
  ...
  let render = () => {
    if (rap) {
      return rap.danhSachGhe.map((item, index) => (
        <StyledTableRow key={index}>
          <StyledTableCell component="th" scope="row">
            {item.tenHeThongRap}
          </StyledTableCell>
          <StyledTableCell align="right">{item.tenRap}</StyledTableCell>
          <StyledTableCell align="right">{item.tenGhe}</StyledTableCell>
        </StyledTableRow>
      ));
    }
  };
        ...
        <Table className={classes.table} aria-label="customized table">
          <TableHead style={{ textAlign: "center" }}>
            <TableRow>
              ...
              <StyledTableCell align="right">Tên Rạp</StyledTableCell>
              <StyledTableCell align="right">Tên ghế</StyledTableCell>
            </TableRow>
          </TableHead>
          <TableBody>{maVe ? render() : ""}</TableBody>
        </Table>
      </TableContainer>
    </div>
  );
    ...
    if (arrBook.length === 0) {
      ...
      return arrBook.map((item, index) => (
        <StyledTableRow key={index}>
          ...
          <StyledTableCell align="right">{item.tenPhim}</StyledTableCell>
      ...
      <React.Fragment>
        <TableContainer component={Paper}>
          <Table className={classes.table} aria-label="customized table">
            ...
            <TableBody>{renderBooking()}</TableBody>
          </Table>
        </TableContainer>
      </React.Fragment>
    );
